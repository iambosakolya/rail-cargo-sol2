    ...
    def show_current_step():
        ...
        if window_number == 1:
            ...
            def save_cargo():
                ...
                cargo_obj = CargoType(cargo_name, description, dimensions, quantity_input.get(), weight_input.get())
            ...
            def create(cursor, client_id, payment_id, dispatcher_id, cargo_id, itinerary_id):
                ...
                try:
                    ...
                    CTkMessagebox(message="Error: " + str(e), icon="cancel", option_1="OK")
                    return None
            def enter_pib():
                    ...
                    try:
                        ...
                        if dispatcher_data:
                            ...
                            if client_data:
                                ...
                                contract_id = create(cursor, client_id, payment_id, dispatcher_id, cargo_id,
                                                              itinerary_id)
                                if contract_id:
                            ...
                            CTkMessagebox(message="Dispatcher with provided PIB not found!", icon="cancel",
                                          option_1="OK")
                    except sqlite3.Error as e:
                        CTkMessagebox(message="Database error: " + str(e), icon="cancel", option_1="OK")
            ...
            savec_btn = CTkButton(master=screen_frame, text="Save contract", **btn_style, command=enter_pib)
            ...
            def print_contract(contract_id):
                try:
                    ...
                    contract_data, client_data, payment_data, dispatcher_data, cargo_data, cargo_type_data, itinerary_data = get_contract_data(
                        cursor, contract_id)
                    ...
                    dispatcher_info = f"\nPIB: {dispatcher_data[1]}" if dispatcher_data else "Dispatcher information not available"
                    cargo_info = (
                        f"\nCargo type: {cargo_type_data[1]}\nDescription: {cargo_type_data[2]}\nDimensions: {cargo_type_data[3]} metres\nQuantity: {cargo_data[2]}\nWeight: {cargo_data[3]} tons") if cargo_data and cargo_type_data else "Cargo information not available"
                    itinerary_info = (
                        f"\nDeparture station: {itinerary_data[1]}\nArrival station: {itinerary_data[2]}\nRoute length: {itinerary_data[3]} km\nDuration: {itinerary_data[4]} hour(s)") if itinerary_data else "Itinerary information not available"
            ...
            def print_button(contract_id):
                print_btn = CTkButton(master=screen_frame, text="Print contract", **btn_style,
...
def update_contract():
    ...
    cargo_btn = CTkButton(master=screen_frame, text="Change contract info", **btn_style,
                          command=modifying_contract(cargo_data, cargo_type_data, itinerary_data))
    cargo_btn.place(relx=0, rely=0.1, anchor="w", x=240, y=350)

    update_window.mainloop()
def modifying_contract(cargo_data, cargo_type_data, itinerary_data):
    global screen_frame
    window_number = 1
    cont_window = CTk()
    ...
    def next_step():
        nonlocal window_number
        window_number += 1
        show_current_step(cargo_data, cargo_type_data, itinerary_data)

    def show_current_step(cargo_data, cargo_type_data, itinerary_data):
        global screen_frame
        screen_frame.destroy()
        screen_frame = CTkFrame(master=cont_window, width=850, height=750, fg_color="#897E9B")
        screen_frame.pack_propagate(0)
        ...
        if window_number == 1:
            ...
            def record_payment(calculated_tariff):
                try:
                    ...
                    # Знайти останній запис у таблиці Payment
                    cursor.execute("SELECT payment_id FROM Payment ORDER BY payment_id DESC LIMIT 1")
